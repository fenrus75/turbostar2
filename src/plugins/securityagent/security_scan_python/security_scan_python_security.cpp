...
bool security_scan_python_validator::validate_args_impl(const nlohmann::json &args, const agentlib::tool_context &ctx,
...
std::unique_ptr<agentlib::llm_tool> security_scan_python_validator::create_tool_impl(const nlohmann::json & /*args*/) const
{
	return std::make_unique<security_scan_python_tool>(resolved_paths_);
}
...
{
	if (std::filesystem::exists("/usr/bin/bandit")) {
		agentlib::tool_registry::get_instance().register_validator(
		    []() { return std::make_unique<tools::security_scan_python_validator>(); });
	}
}

void unregister_security_scan_python(void)
{
	agentlib::tool_registry::get_instance().unregister_validator("security_scan_python");
}
}
