...
class html_list_links_validator : public agentlib::tool_validator
{
      public:
	html_list_links_validator() = default;
	~html_list_links_validator() override = default;

	bool is_pure() const override { return true; }
	std::string get_name() const override { return "html_list_links"; }
	std::string get_description() const override
	{
		return "Extracts all hyperlink anchor text and URLs from an HTML document.";
	}
	...
	std::unique_ptr<agentlib::llm_tool> create_tool_impl(const nlohmann::json &raw_json) const override;
      ...
      private:
	mutable html_list_links_args args_;
};
