...
void find_links(lxb_html_document_t *document, lxb_dom_node_t *node, std::vector<std::pair<std::string, std::string>> &links)
{
	if (!node)
		return;
...
std::string html_list_links_tool::execute(agentlib::tool_context & /*ctx*/)
{
	...
	std::string tbl = "### Links extracted from: " + args_.requested_path + "\n\n";
	tbl += "| Text | URL |\n| --- | --- |\n";
