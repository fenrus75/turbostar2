...
- **Crashdump Settling & Utility Command Isolation**: When a child process terminates with a non-zero exit code and crash interception is enabled, `libturbocatch.so` writes post-mortem files (registers, stack, core dump) asynchronously before process termination. Pausing briefly (~50ms) in `on_child_exit()` gives disk writes time to settle before `crashdump_manager::refresh()` scans for new dumps. Additionally, internal synchronous command runners (e.g. `execute_command_sync`) must set `bypass_crashdump_check_ = true` so internal background queries (such as git blob hashing) do not prematurely refresh or steal crash dumps intended for the foreground runner.
