...
bool diff_window::process_events()
{
	...
	while (auto ev = get_queue().pop()) {
		...
		if (prev_button_->handle_event(*ev, x_ + prev_button_->x(), y_ + prev_button_->y()))
			needs_render = true;
		if (next_button_->handle_event(*ev, x_ + next_button_->x(), y_ + next_button_->y()))
			needs_render = true;
		if (restore_button_->handle_event(*ev, x_ + restore_button_->x(), y_ + restore_button_->y()))
			needs_render = true;
	}
...
void diff_window::update_diff()
{
	...
	while (context < 20) {
		...
		if ((int)diff_lines_.size() >= available_height)
			break;
...
void diff_window::on_resize(int /*width*/, int height)
{
	if (prev_button_) {
		prev_button_->set_bounds(2, height - 2, prev_button_->width(), prev_button_->height());
	}
	if (next_button_) {
		next_button_->set_bounds(15, height - 2, next_button_->width(), next_button_->height());
	}
	if (restore_button_) {
		restore_button_->set_bounds(30, height - 2, restore_button_->width(), restore_button_->height());
	}
	update_diff();
}
