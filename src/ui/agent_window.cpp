    ...
    : window(id, x, y, width, height, "Agent Chat")
{
	...
	std::string system_prompt =
	    ...
	    "STRONGLY PREFER using built-in tools (e.g., fs_read_lines, fs_find_in_files, fs_replace_lines, sqlite_perform, git_status, git_diff_unstaged, git_diff_staged, fs_list_tests) over the generic "
	    "`run_shell_command` tool (e.g., use `fs_find_in_files` instead of running `grep` via the shell, use `git_diff_unstaged` / `git_diff_staged` instead of running `git diff` via the shell, and use `fs_list_tests` instead of `meson test --list` or `ctest --show-only` via the shell).\n"
