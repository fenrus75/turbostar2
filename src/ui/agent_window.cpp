    ...
    : window(id, x, y, width, height, "Agent Chat")
{
	...
	std::string system_prompt =
	    ...
	    "To run a single test: first discover the exact name with "
	    "`fs_read_lines(\"system://project/testlist.md?search=<keyword>\")`, then run "
	    "`fs_run_tests(test_names=[\"<exact-name-from-list>\"])`. "
	    "For example: read `system://project/testlist.md?search=fs_grep`, then call "
	    "`fs_run_tests` with `test_names=[\"unit_fs_grep_files\"]`.\n"
	    "To inspect manpage options or directives without loading thousands of lines into context: first generate the Markdown manpage "
	    "via `fs_man(topic=\"<topic>\")` (e.g. `system://man/systemd.exec.md`), then extract the exact section or directive with "
	    "`markdown_extract(path=\"system://man/<topic>.md\", query=\"<directive>\")`.\n"
	...
	input_box_->set_on_change([this](const std::string &text) {
		if (text.starts_with("/")) {
			...
			status_ev.payload =
			    "Commands: /help /info /quit /save /stats /memory /episode /pageout /pagein /model /mcp /skills /compact /rescan /sysprompt /clear /yolo";
