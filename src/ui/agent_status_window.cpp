...
agent_status_window::agent_status_window(int id, int x, int y, int width, int height, const std::string &title,
					 std::shared_ptr<agentlib::ai_agent> agent, event_queue &global_queue)
...
void agent_status_window::set_cursor_position() const
{
	if (is_active()) {
		...
		if (target_y > y_ + height_ - 2)
			target_y = y_ + height_ - 2;
		move(target_y, x_ + 1);
	} else {
