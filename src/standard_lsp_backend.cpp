...
standard_lsp_backend::standard_lsp_backend()
{
}
...
void standard_lsp_backend::start(event_queue &queue)
{
	global_queue_.store(&queue);
}

void standard_lsp_backend::stop()
{
...
std::shared_ptr<standard_lsp_backend::server_instance> standard_lsp_backend::get_server_for_file(const std::string &filepath)
{
	...
	if (lang_id == "cpp") {
		...
		start_server("clangd", {"-log=error", std::format("--compile-commands-dir={}", build_dir), std::format("-j={}", clangd_jobs), "--malloc-trim", "--background-index"},
			     "cpp");
...
bool standard_lsp_backend::is_supported_file(const std::string &filepath) const
{
	std::string ext = fs::path(filepath).extension().string();
	for (auto &c : ext)
		c = std::tolower(c);
...
std::vector<text_range> standard_lsp_backend::query_selection_ranges(const std::string &filepath, int line, int character)
{
...
std::vector<standard_lsp_backend::location_info> standard_lsp_backend::query_definition(const std::string &filepath, int line, int character)
{
	auto server = get_server_for_file(filepath);
	if (!server)
		return {};
...
void standard_lsp_backend::open_document(const std::string &filepath, const std::string &text)
{
	auto server = get_server_for_file(filepath);
	if (!server)
		return;
...
void standard_lsp_backend::request_document_highlight(const std::string &filepath, int line, int character)
{
	auto server = get_server_for_file(filepath);
	if (!server)
		return;
...
void standard_lsp_backend::request_selection_range(const std::string &filepath, int line, int character)
{
	auto server = get_server_for_file(filepath);
	if (!server)
		return;
...
std::vector<standard_lsp_backend::symbol_info> standard_lsp_backend::query_workspace_symbols(const std::string &query)
{
...
std::vector<standard_lsp_backend::call_hierarchy_item> standard_lsp_backend::query_call_hierarchy_outgoing(const std::string &filepath, int line,
											 int character)
{
	auto server = get_server_for_file(filepath);
	if (!server)
...
std::vector<standard_lsp_backend::outgoing_call_item> standard_lsp_backend::query_call_hierarchy_outgoing_batch(
...
std::vector<standard_lsp_backend::type_hierarchy_item> standard_lsp_backend::query_type_hierarchy_supertypes(const std::string &filepath, int line,
											   int character)
{
	auto server = get_server_for_file(filepath);
	if (!server)
...
void standard_lsp_backend::store_file_diagnostics(const std::string &filepath, const std::vector<diagnostic_info> &diags)
{
	std::lock_guard<std::mutex> lock(diagnostics_mutex_);
	file_diagnostics_[filepath] = diags;
}

std::optional<std::vector<diagnostic_info>> standard_lsp_backend::query_file_diagnostics(const std::string &filepath)
{
