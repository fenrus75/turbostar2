...
{
      public:
	editor(bool debug_mode, const std::string &debug_string, const std::vector<std::string> &filenames, double exit_delay, bool no_lsp = false);
	~editor();
	...
	enum class dialog_mode { none, load, save, search, replace, insert_file, settings, save_prompt, force_quit_prompt, ask_user, model_list, model_edit, model_selection, welcome };	dialog_mode active_dialog_mode_{dialog_mode::none};
	...
	double exit_delay_{0.0};  // 0.0 = normal operation, >0 = exit after delay in seconds
