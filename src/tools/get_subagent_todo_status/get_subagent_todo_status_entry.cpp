...
get_subagent_todo_status_tool::get_subagent_todo_status_tool(get_subagent_todo_status_args args) : args_(std::move(args)) {}

bool get_subagent_todo_status_tool::validate_runtime(const agentlib::tool_context& ctx, std::string& out_error) const {
    if (!ctx.active_agent) {
...
std::string get_subagent_todo_status_tool::execute(agentlib::tool_context& ctx) {
