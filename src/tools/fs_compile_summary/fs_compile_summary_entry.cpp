...
std::string fs_compile_summary_tool::execute(agentlib::tool_context& ctx) {
    ...
    for (const auto& raw_path : unique_paths_to_check) {
        ...
        if (rel_str.empty() || rel_str == ".") rel_str = safe_path; // Fallback to absolute if something weird happens
        ...
        for (const auto& err : build_errors) {
            ...
            if (ctx.fs_security.validate_access(err.filepath, agentlib::access_type::read, err_safe, err_error) && err_safe == safe_path) {
                if (err.is_warning) stats.compiler_warnings++;
                else stats.compiler_errors++;
                has_issues = true;
            }
        }
        ...
        if (ctx.doc_provider) {
            ...
            if (doc_snapshot) {
                ...
                for (const auto& d : diagnostics) {
                    if (d.severity == "Error") stats.lsp_errors++;
                    else if (d.severity == "Warning") stats.lsp_warnings++;
