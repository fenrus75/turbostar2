...
public:
    std::string get_name() const override { return "fs_regexp_lines"; }
    std::string get_description() const override { return "Search for a regular expression within a file and return matching lines as a Markdown table."; }
    ...
    nlohmann::json get_parameters_schema() const override {
        return {
            ...
            {"properties", {
                ...
                {"pattern", {
                    {"type", "string"},
                    {"description", "The C++ std::regex pattern to search for (e.g., 'function.*foo')."}
                }}
            }},
