...
std::string fs_compile_info_tool::execute(agentlib::tool_context& ctx) {
    ...
    for (const auto& err : errors) {
        ...
        if (err.filepath.find(requested_path_) != std::string::npos || 
            safe_path_.find(err.filepath) != std::string::npos) 
        {
            found_compile_errors = true;
            compile_table << "| " << err.line << " | " << err.column << " | " 
                          << (err.is_warning ? "Warning" : "Error") << " | " 
