...
std::string code_get_references_tool::execute(agentlib::tool_context& ctx) {
    ...
    auto locations = lsp_manager::get_instance().query_references(safe_path, args_.line - 1, args_.character);
    if (locations.empty()) {
...
public:
    std::string get_name() const override { return "code_get_references"; }
    std::string get_description() const override { 
        return "Finds all references (usages) of a symbol across the project."; 
    }
...
protected:
    ...
    std::unique_ptr<agentlib::llm_tool> create_tool_impl(const nlohmann::json& args) const override {
        code_get_references_args parsed_args;
        parsed_args.path = args["path"].get<std::string>();
        parsed_args.line = args["line"].get<int>();
        parsed_args.character = args["character"].get<int>();
        return std::make_unique<code_get_references_tool>(std::move(parsed_args));
    }
};

REGISTER_TOOL(code_get_references_validator);
