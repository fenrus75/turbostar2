...
class get_subagent_status_validator : public agentlib::tool_validator
{
	...
	{
		return "Returns detailed status information about a specific subagent.";
	}
	...
	{
		return std::make_unique<get_subagent_status_tool>(args_);
	}

      private:
	mutable get_subagent_status_args args_;
};

REGISTER_TOOL(get_subagent_status_validator)
