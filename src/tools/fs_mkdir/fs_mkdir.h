...
public:
    ...
    std::string get_description() const override { return "Create a directory, including any necessary parent directories (like mkdir -p). Supports workspace relative paths or virtual URIs (e.g., 'tmp://dir'). Use this tool when creating directory structures before writing files, instead of running shell 'mkdir' commands.";; }
