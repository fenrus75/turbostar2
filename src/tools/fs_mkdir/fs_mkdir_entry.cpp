...
bool fs_mkdir_tool::validate_runtime(const agentlib::tool_context &ctx, std::string &out_error) const
{
	...
	if (!ctx.fs_security.validate_access(safe_path_, agentlib::access_type::write, canonical_path, out_error)) {
		return false;
	}
...
std::string fs_mkdir_tool::execute(agentlib::tool_context &ctx)
{
	if (safe_path_.find("://") != std::string::npos) {
		...
		if (vfs->create_directory(safe_path_)) {
			...
			return fs_utils::wrap_prompt_untrusted_data_tag("fs_mkdir_result", "Successfully created directory in virtual file system: " + safe_path_);
		} else {
	...
	try {
		...
		if (created) {
			...
			return fs_utils::wrap_prompt_untrusted_data_tag("fs_mkdir_result", "Successfully created directory: " + safe_path_);
		} else {
			set_success(ctx, "Directory already existed");
			return fs_utils::wrap_prompt_untrusted_data_tag("fs_mkdir_result", "Directory already exists: " + safe_path_);
		}
