...
shell_command_recommendation evaluate_shell_command_resteer(const std::string &command_line)
{
	...
	static const std::regex test_list_grep_regex(
	    R"(^(?:meson\s+test.*--list|ctest.*(?:-N|--show-only)).*\|\s*grep\s+(?:-i\s+)?["']?([^"'\s]+)["']?)", std::regex::icase);
	static const std::regex test_list_regex(R"(^(?:meson\s+test.*--list|ctest.*(?:-N|--show-only)|ninja\s+-t\s+targets))",
						std::regex::icase);
	std::smatch match;
	...
	if (std::regex_search(cmd, match, test_list_grep_regex)) {
		...
		rec.explanation = std::format("To discover test names matching '{}', read the live system://project/testlist.md VFS file "
					      "using fs_read_lines instead of running test list commands via shell.",
					      kw);
		return rec;
	}
	if (std::regex_search(cmd, test_list_regex)) {
		...
		rec.explanation = "To discover available test names, read the live system://project/testlist.md VFS file using "
				  "fs_read_lines instead of running test list commands via shell.";
		return rec;
	}

	// 2. Unit Test Execution: meson test [options] <testname> or ./build/test_* or ninja <target>
	static const std::regex meson_test_exec_regex(R"(^meson\s+test\s+(?:--print-errorlogs\s+|-C\s+\S+\s+|--verbose\s+)*([\w_]+)$)",
						      std::regex::icase);
	...
	if (std::regex_match(cmd, match, meson_test_exec_regex) || std::regex_match(cmd, match, direct_binary_exec_regex) ||
	    std::regex_match(cmd, match, ninja_test_exec_regex)) {
		...
		rec.explanation = std::format(
		    "To run unit tests like '{}', execute them using the native 'fs_run_tests' tool instead of raw shell commands.", tname);
		return rec;
	}
	...
	if (std::regex_match(cmd, match, sed_read_regex)) {
		...
		rec.explanation =
		    std::format("To read lines from '{}', use the native 'fs_read_lines' tool instead of shell sed.", file_path);
		return rec;
	}
	if (std::regex_match(cmd, match, head_read_regex)) {
		...
		rec.explanation =
		    std::format("To read top lines from '{}', use the native 'fs_read_lines' tool instead of shell head.", file_path);
		return rec;
	}

	// 4. Code / Workspace Search: grep -rn "pat" dir, grep -n "pat" file, find . -name "pat"
	static const std::regex grep_search_regex(R"(^grep\s+-[a-zA-Z]*[rn][a-zA-Z]*\s+["']?([^"'\s]+)["']?(?:\s+([^\s\|;]+))?)",
						  std::regex::icase);
	...
	// 5. Git Status / Diff / Log / Show / Ls-files: git status, git diff, git log, git show, git ls-files
	...
	static const std::regex git_show_regex(R"(^git\s+show(?:\s+.*)?$)");
	...
	if (std::regex_match(cmd, git_show_regex)) {
		rec.matched = true;
		rec.confidence = 0.90;
		bool has_stat = (cmd.find("--stat") != std::string::npos);

		// Look for revision argument (token not starting with '-')
		...
		while (iss >> token) {
			...
			if (!token.starts_with("-")) {
				commit_id = sanitize_for_prompt_quote(token);
				break;
			}
		}
		...
		if (has_stat) {
			if (!commit_id.empty()) {
				rec.suggested_tool = std::format("git_log(commit_id=\"{}\", stat=true)", commit_id);
			} else {
				rec.suggested_tool = "git_log(stat=true)";
			}
			rec.explanation =
			    "To view commit diffstat or details, use the native 'git_log' tool with stat=true instead of shell git show.";
		} else {
			if (!commit_id.empty()) {
				rec.suggested_tool = std::format("git_log(commit_id=\"{}\", show_patch=true)", commit_id);
			} else {
				rec.suggested_tool = "git_log(show_patch=true)";
			}
			rec.explanation = "To view commit details or unified diffs, use the native 'git_log' tool with show_patch=true "
					  "instead of shell git show.";
		}
		return rec;
	}
	if (std::regex_match(cmd, git_log_regex)) {
		rec.matched = true;
		rec.confidence = 0.90;
		bool has_patch = (cmd.find("-p") != std::string::npos || cmd.find("--patch") != std::string::npos);
		bool has_stat = (cmd.find("--stat") != std::string::npos);

		// Look for revision argument (token not starting with '-' and not a digit limit)
		std::string commit_id;
		std::istringstream iss(cmd);
		std::string token;
		iss >> token; // "git"
		...
		if (has_patch) {
			...
			rec.explanation = "To view commit history with unified diffs, use the native 'git_log' tool with show_patch=true "
					  "instead of shell git log.";
		} else if (has_stat) {
			if (!commit_id.empty()) {
				rec.suggested_tool = std::format("git_log(commit_id=\"{}\", stat=true)", commit_id);
			} else {
			...
			rec.explanation =
			    "To view commit history with diffstat, use the native 'git_log' tool with stat=true instead of shell git log.";
		} else {
			if (!commit_id.empty()) {
				rec.suggested_tool = std::format("git_log(commit_id=\"{}\")", commit_id);
			} else {
				rec.suggested_tool = "git_log(limit=10)";
			}
			rec.explanation =
			    "To view commit history or commit details, use the native 'git_log' tool instead of shell git log.";
		}
		return rec;
	}
	...
	if (std::regex_match(cmd, match, cat_read_regex)) {
		...
		rec.explanation =
		    std::format("To view file contents of '{}', use the native 'fs_read_lines' tool instead of shell cat.", file_path);
		return rec;
	}
	...
	if (std::regex_search(cmd, match, curl_exec_regex)) {
		...
		rec.explanation = std::format(
		    "To fetch or test HTTP/HTTPS URLs like '{}', use the native 'web_fetch' tool instead of shell curl.", target_url);
		return rec;
	}
	...
	if (std::regex_match(cmd, match, sqlite_exec_regex)) {
		...
		rec.explanation =
		    std::format("To query SQLite database '{}', use the native 'sqlite_perform' tool instead of shell sqlite3.", db_file);
