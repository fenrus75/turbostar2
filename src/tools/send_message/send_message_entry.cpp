...
send_message_tool::send_message_tool(send_message_args args) : args_(std::move(args)) {}

bool send_message_tool::validate_runtime(const agentlib::tool_context& ctx, std::string& out_error) const {
    if (!ctx.active_agent) {
...
std::string send_message_tool::execute(agentlib::tool_context& ctx) {
