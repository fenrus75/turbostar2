...
{
	if (!args.contains("filename") || !args["filename"].is_string() || !args.contains("line") || !args["line"].is_number() ||
		...
		out_error = "Invalid arguments: Requires filename (string), line (int), column (int), length (int), error_string (string), "
			    "is_warning (bool).";
		return false;
	}
...
std::unique_ptr<agentlib::llm_tool> flag_as_error_security::create_tool_impl(const nlohmann::json &args) const
{
	...
	// assuming execution happens correctly. Wait, validate_path is non-const, let's just create the tool with raw filename and execute
	// can resolve. Actually, create_tool_impl is const, and ctx is not passed. Let's just pass the raw args and let the tool
	// re-resolve, or do the resolution here if we can. Wait, the new convention is `tool` executes and gets `ctx`!
