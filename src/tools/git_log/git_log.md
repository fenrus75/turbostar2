...
- **Global Parameter Aliases**: Inherits global alias normalization from `tool_validator` (e.g., `file_path`, `filepath`, `filename`, `file`, `target_file` are automatically mapped to `path`, and `count`, `max_results` are mapped to `limit`).
- **Security & Sandboxing**:
  - Validates `path` using `ctx.fs_security.validate_access(untrusted_path, agentlib::access_type::read, safe_path, out_error)`.
  - Escapes `safe_path` using `fs_utils::escape_shell_arg` when executing the git subprocess.
