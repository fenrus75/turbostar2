    ...
    std::vector<agentlib::interaction_line> format_lines(int width) const override {
        ...
        if (!diff_lines_.empty()) {
            ...
            for (const auto& dl : diff_lines_) {
                ...
                if (dl[0] == '-') color = 31; // Bright Red on Dark Blue
                else if (dl[0] == '+') color = 30; // Bright Green on Dark Blue
                else if (dl.length() > 2 && dl[0] == '@' && dl[1] == '@') color = 32; // Bright Cyan on Dark Blue
...
std::string fs_replace_lines_tool::execute(agentlib::tool_context& ctx) {
    ...
    if (ctx.doc_provider && ctx.doc_provider->get_open_document(args_.safe_path)) {
        ...
        // Let's fallback to disk if we want the diff. Actually, doc_provider->apply_live_edits could be synchronous, but reading before/after is safer via disk fallback for the agent's view.
        // For the sake of this feature, we will execute the disk fallback first to compute the diff and then still apply the live edit if open.
    }
