...
bool fs_replace_lines_validator::validate_args_impl(const nlohmann::json& raw_json, const agentlib::tool_context& ctx, std::string& out_error) const {
    try {
        ...
        for (const auto& edit_json : raw_json["edits"]) {
            ...
            if (edit.type == "remove" || edit.type == "replace") {
                if (!edit_json.contains("original_text") || !edit_json["original_text"].is_string()) {
                    out_error = "'original_text' parameter is REQUIRED for '" + edit.type + "' operations. You must provide it.";
                    return false;
                }
            ...
            if (edit.type == "add" || edit.type == "replace") {
                if (!edit_json.contains("replace_with") || !edit_json["replace_with"].is_string()) {
                    out_error = "'replace_with' parameter is REQUIRED for '" + edit.type + "' operations. You must provide it.";
                    return false;
                }
