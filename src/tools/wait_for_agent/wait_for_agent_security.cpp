...
public:
    ...
    std::string get_name() const override { return "wait_for_agent"; }
    std::string get_description() const override { return "Pauses your execution until the specified subagent finishes its current task and returns to an idle state."; }
    
    nlohmann::json get_parameters_schema() const override {
        return {
            {"type", "object"},
            {"properties", {
                {"id", {{"type", "integer"}, {"description", "The ID of the subagent to wait for."}}}
            }},
