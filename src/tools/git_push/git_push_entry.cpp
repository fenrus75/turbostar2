...
git_push_tool::git_push_tool(bool force) 
    : llm_tool_action("Pushing to remote"), force_(force) {}
...
std::string git_push_tool::execute(agentlib::tool_context& ctx) {
    ...
    if ((output.find("fatal:") == std::string::npos && output.find("error:") == std::string::npos && output.find("Everything up-to-date") != std::string::npos) || output.find("->") != std::string::npos || output.find("forced update") != std::string::npos) {
