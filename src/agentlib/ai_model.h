...
class ai_model {
public:
    ai_model(std::string id, std::string name, std::string url, std::string purpose, double cost_per_1m_tx, double cost_per_1m_rx, std::string api_key = "")
        : id_(std::move(id)), name_(std::move(name)), url_(std::move(url)), purpose_(std::move(purpose)), 
          cost_per_1m_tx_(cost_per_1m_tx), cost_per_1m_rx_(cost_per_1m_rx), api_key_(std::move(api_key)) {}
