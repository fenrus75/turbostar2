...
void llm_client::send_chat_stream(const std::vector<message>& conversation, 
    ...
    bool success = transport_->post_stream("/v1/chat/completions", body, [&](const char* data, size_t len, size_t /*off*/, size_t /*total*/) {
            ...
            if (line.rfind("data: ", 0) == 0) {
                ...
                try {
                    ...
                    if (chunk.contains("usage") && !chunk["usage"].is_null()) {
                        ...
                        if (usage.contains("prompt_tokens")) delta.usage.prompt_tokens = usage["prompt_tokens"].get<int>();
                        if (usage.contains("completion_tokens")) delta.usage.completion_tokens = usage["completion_tokens"].get<int>();
                        if (usage.contains("total_tokens")) delta.usage.total_tokens = usage["total_tokens"].get<int>();
                    }
                    ...
                    if (chunk.contains("choices") && !chunk["choices"].empty()) {
                        ...
                        if (choice.contains("delta")) {
                            auto d = choice["delta"];
                            if (d.contains("role")) delta.role = d["role"].get<std::string>();
                            if (d.contains("content") && !d["content"].is_null()) delta.content = d["content"].get<std::string>();
