					...
					agent_role role)
{
	...
	std::string body = formatter_->build_chat_payload(model_id_, normalized, registry, false, active_families, previous_response_id, role);
...
void llm_client::send_chat_stream(const std::vector<message> &conversation, std::function<void(const chat_delta &)> callback,
				  ...
				  const std::string &previous_response_id, agent_role role)
{
	std::vector<message> normalized = normalize_conversation_history(conversation);
	std::string body = formatter_->build_chat_payload(model_id_, normalized, registry, true, active_families, previous_response_id, role);
