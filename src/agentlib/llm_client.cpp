					...
					agent_role role)
{
	...
	}, nullptr, active_families, previous_response_id, role);
				  ...
				  const std::string &previous_response_id, agent_role role)
{
	...
	std::string identity = "developer";
	if (role == agent_role::reviewer) {
		identity = "reviewer";
	} else if (role == agent_role::verifier) {
		identity = "verifier";
	} else if (role == agent_role::summarizer) {
		identity = "summarizer";
	}

	connection_->send_prompt(*convo, identity, active_families, [callback](const stream_event& ev) {
		chat_delta delta;
