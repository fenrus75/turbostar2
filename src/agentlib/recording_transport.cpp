...
transport_response recording_transport::post(const std::string& path, const std::string& json_body) {
    ...
    json interaction = {
        ...
        {"response", {
            ...
            {"body", json::parse(res.body, nullptr, false)}
        }}
    };

    // Fallback if not valid JSON
    if (interaction["request"].is_discarded()) interaction["request"] = request_body;
    if (interaction["response"]["body"].is_discarded()) interaction["response"]["body"] = res.body;
