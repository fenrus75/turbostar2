...
- **Live LLM Test Timeouts with Large Models**: When running tests under `TURBOSTAR_IN_TESTSUITE=1`, timeouts were previously clamped to 5s connect / 15s read for fast local unit mock tests. However, live integration tests against real inference servers (such as large 284B parameter models like MiniMax generating at ~21 tokens/s) require substantial prompt prefill and generation time. Bypassing the artificial 15s clamp when `TURBOSTAR_LIVE_LLM_TEST` is active and granting adequate test harness and watchdog timeouts prevents premature aborts.
