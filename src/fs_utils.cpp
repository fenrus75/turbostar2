...
std::filesystem::path safe_absolute(const std::filesystem::path &p)
{
	...
	try {
		...
		event_logger::get_instance().log(std::format("Filesystem error resolving absolute path for '{}': {}", p.string(), e.what()));
		return p.lexically_normal();
	} catch (...) {
		event_logger::get_instance().log(std::format("Unknown error resolving absolute path for '{}'", p.string()));
		return p.lexically_normal();
	}
}
