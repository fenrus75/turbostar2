...
- **Sandbox Transient Mounts Require Absolute Paths**: When `systemd-run` sets up bind mounts (e.g. for temporary or cache directories via `add_extra_rw_path`), systemd strictly requires source paths to be absolute (otherwise rejecting transient services with `Source path ... is not absolute`). `get_project_tmp_dir()` must always return a normalized absolute path, falling back to `current_path()` when `get_project_dir()` is uninitialized or empty.
