...
class config_manager
{
	...
	{
		return clang_format_style_;
	}
	void set_clang_format_style(const std::string &style)
	{
		clang_format_style_ = style;
	}
	...
	void set_build_directory(const std::string &dir)
	{
		build_directory_ = dir;
	}
	...
	void set_auto_open_error_files(bool auto_open)
	{
		auto_open_error_files_ = auto_open;
	}

	bool is_compile_on_save() const
	{
		return compile_on_save_;
	}
	void set_compile_on_save(bool compile)
	{
	...
	void set_default_model_id(const std::string &id)
	{
	...
	void set_log_all_tool_calls(bool log_all)
	{
	...
	void set_run_arguments(const std::string &args)
	{
		run_arguments_ = args;
	}

	std::string get_run_target_mode() const
	{
		return run_target_mode_;
	}
	void set_run_target_mode(const std::string &mode)
	{
		run_target_mode_ = mode;
	}

	bool get_gdb_auto_continue() const
	{
		return gdb_auto_continue_;
	}
	void set_gdb_auto_continue(bool val)
	{
		gdb_auto_continue_ = val;
	}

	bool is_mcp_server_enabled(const std::string &server_name, bool is_system, bool default_val = true) const;
	void set_mcp_server_enabled(const std::string &server_name, bool is_system, bool enabled);

	bool is_mcp_tool_enabled(const std::string &server_name, const std::string &tool_name, bool is_system,
				 bool default_val = true) const;
	void set_mcp_tool_enabled(const std::string &server_name, const std::string &tool_name, bool is_system, bool enabled);
	...
	std::map<std::string, bool> project_mcp_servers_enabled_;
