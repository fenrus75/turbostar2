...
std::string file_dialog::get_autocomplete_suggestion() const {
	if (!autocomplete_ || filename_buffer_.empty()) return "";

	// Check if there is already an exact match, in which case we don't suggest a longer string
...
void file_dialog::draw_button(int by, int bx, const std::string& btext, char bhot, bool focused) const {
	...
	for (size_t i = 0; i < btext.length(); ++i) mvaddstr(by + 1, bx + 1 + i, "▀");
...
void file_dialog::draw() const
{
	...
	for (int i = 0; i < list_box_height; ++i) {
		...
		for (int j = 0; j < list_box_w; ++j) addch(' ');
	...
	addstr("◄");
	for (int j = 1; j < list_box_w - 1; ++j) addstr("░");
	addstr("►");
	if (!files_.empty()) {
		...
		if (thumb_pos >= list_box_w - 1) thumb_pos = list_box_w - 2;
	...
	if (!active_name.empty()) {
		...
		for (const auto& fe : files_) {
			if (fe.display_name == active_name || (fe.display_name.substr(0, fe.display_name.length()-1) == active_name && fe.is_dir)) {
...
dialog_result file_dialog::handle_key(int key)
{
	...
	if (history_dropdown_open_) {
		if (key == KEY_UP && history_sel_idx_ > 0) history_sel_idx_--;
		else if (key == KEY_DOWN && history_sel_idx_ < static_cast<int>(file_history_.size()) - 1) history_sel_idx_++;
	...
	if (key == '\t' || key == KEY_BTAB) {
		...
		if (key == '\t') current = (current + 1) % num_elements;
		else current = (current - 1 + num_elements) % num_elements;
	...
	if (focus_ == focus_element::entry_box) {
		if (key >= 32 && key <= 126) filename_buffer_ += static_cast<char>(key);
			...
			std::string suggestion = get_autocomplete_suggestion();
			if (!suggestion.empty()) filename_buffer_ = suggestion;
		} else if (key == KEY_ENTER || key == 10 || key == 13) {
			if (filename_buffer_.empty()) return dialog_result::pending;
		...
		switch (key) {
			...
			case KEY_LEFT:
				...
				break;
			case KEY_RIGHT:
				if (selected_index_ + files_height < static_cast<int>(files_.size())) selected_index_ += files_height;
		...
		if (key == ' ' || key == KEY_ENTER || key == 10 || key == 13) return dialog_result::cancelled;
	}
