...
3.  **Context Restoration (Virtual Paging) [IMPLEMENTED (Backend)]:** By providing a dedicated tool like `agent_restore_milestone_context(milestone_id)`, the agent gains the ability to perform a true "task switch." If it needs to resume work on a deeply historical task, it can call this tool. Assuming the token budget allows, the C++ backend will deserialize the archive and natively re-inject the historical conversational turns back into the agent's active memory array, operating exactly like virtual memory paging for an OS.
    *   *Non-Destructive Optimized Page-In (Optional):* When paging in historical context, a `compression_level` (0-3) can be provided to seamlessly optimize the archive as it is injected. Because the underlying archive on disk remains completely untouched, this optimization is non-destructive—the agent can always issue a second command to re-page the raw, uncompressed archive if it realizes it needs deeper details.
        *   **Level 0 (Raw):** 100% uncompressed restoration.
        *   **Level 1 (Think-Free):** Strips explicit `reasoning_content` and pseudo-reasoning from the assistant's tool-call turns. Restores the factual history of actions taken without the massive token bloat of the old chain-of-thought processes.
        *   **Level 2 (Terminal Truncated):** Applies Level 1 + squashes massive historical terminal outputs (e.g., test runs, compilers) down to their Head/Tail screenshots.
        *   **Level 3 (Highly Aggressive):** Applies Level 2 + Semantic Deduplication (removes stale, superseded file reads or listing commands if newer ones exist).
    *   *Progressive Stripping (Think-Free Tiers):* Because different LLM providers handle "thinking" differently, the Level 1 optimized page-in utilizes a two-tiered heuristic. **Tier 1** strips explicit `reasoning_content` fields (used by models like DeepSeek). **Tier 2** strips conversational `content` *only* if the turn also contains a `tool_call`. This perfectly eliminates pseudo-reasoning bloat from models like GPT-4o (e.g., "I will now check the file..."), while safely preserving actual conversational text from turns where the agent was directly talking to the user.
...
*   **Automated Decision Engine (Memory Eviction) [IMPLEMENTED]:** Managed by the background `compaction_engine` (`src/agentlib/compaction_engine.h/cpp`). The engine runs on every agent turn loop cycle (`evaluate_compaction` in `ai_agent.cpp`), evaluating the active history's token size (using a `characters / 4` approximation) against configured limits (defaulting to 80% upper bound trigger, 40% lower bound target). When the trigger threshold is exceeded, the engine sorts active episodes by oldest LRU access sequence and progressively downgrades them (Level 0 -> Level 1 -> Level 2 -> Level 99/evicted) to reach the target budget.
