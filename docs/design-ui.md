...
- **`ui_durmovie`**: An animation and artwork widget that loads Durdraw `.json` movie files. It parses UTF-8 frame characters, maps 16-color DOS color palettes to NCurses color pairs/bold attributes, and updates cycling animations.
...
- **`ui_thumbnail`**: A widget for rendering image previews in ncurses using Unicode vertical half-block (`▄`) characters. It queries the `"image_thumbnail"` filter registered by the optional `image-basic` plugin, receiving a downsampled pixel grid in JSON format, and maps colors to the nearest standard 256-color terminal cube. If the plugin is not loaded, it falls back to a clean text-based placeholder.
