...
- Multi-threaded:
  ...
  - Synchronization: `std::shared_mutex` for data safety in key datastructures.
- Cursor mapping: Logical character index <-> UTF-8 byte offset <-> Screen column (taking into account tabs and multi-byte characters).
...
- **Methods**: High-level operations like `insert_char()`, `delete_line()`, `load_from_file()`, `save()`.
...
- **Focus**: Knows if it is the active window receiving input.
...
- **Windows**: Manages the layout of `Window` objects on the screen.
