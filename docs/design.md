...
- **In-Slice Call Candidate Extraction (`call_token_extractor`)**: In backends that do not implement `prepareCallHierarchy` / `outgoingCalls` (such as `semcode-lsp`), `codemap_utils::get_outgoing_calls_in_range` falls back to `extract_outgoing_calls_from_slice`. This scans candidate invocation tokens (`func(...)`, `obj->func(...)`, `ns::func(...)`) using `tools::call_token_extractor`, filters language keywords and operators, and queries `textDocument/definition` (which `semcode` supports natively) to resolve caller targets and populate the `### Called Dependencies:` table.
