...
- **`semcode_backend` Implementation** (`src/semcode_backend.h`, `src/semcode_backend.cpp`): Specialized backend inheriting from `standard_lsp_backend`. Automatically activated on demand when a `.semcode.db` database is detected in the project root and `semcode-lsp` is available. Employs a hybrid architecture:
  ...
  - **CLI Query Engine (`semcode`)**: Powers rich semantic type inspection (`type`), function signatures (`func`), call hierarchies (`calls -v`), and workspace symbol discovery. Filters out ambiguous definitions (e.g. ubiquitous macros and multi-architecture definitions like `memset` or `BIT`) and cross-architecture targets (`arch/`) directly within `semcode_backend` queries with strict uniqueness checks and deadline guards. Falls back to `fallback_find_symbols` for per-file document symbols.
  - **Parallel Callee Pre-Warming & In-Flight Deduplication**: For outgoing call hierarchy queries, candidate callees needing definition checks (`func <name>`) are pre-warmed concurrently across detached background threads. Concurrent identical queries are deduplicated via `inflight_queries_` and `inflight_cv_` so redundant subprocesses are never spawned. The main query waits on a shared condition variable up to the request `deadline`; if queries arrive late, background threads continue executing to completion and populate `cli_cache_`, ensuring subsequent queries or navigation requests benefit from instant cache hits without wasted CPU effort.
...
- **Semcode Streaming Indexer & Persistent SQLite Storage (`semcode_indexer`, `turbostar-semcode-indexer`)**:
  - Provides a standalone high-performance binary (`turbostar-semcode-indexer`) and C++ library class (`semcode_indexer`) to ingest massive `semcode` JSON dumps (`dump-functions`, `dump-types`) into a local SQLite database with zero full-DOM JSON allocation.
