...
- **In-Slice Call Candidate Extraction (`call_token_extractor`) & Strict Ambiguity Punting**: In backends that do not implement `prepareCallHierarchy` / `outgoingCalls` (such as `semcode-lsp`), or when call hierarchy misses in-slice calls, `codemap_utils::get_outgoing_calls_in_range` extracts candidates via `extract_outgoing_calls_from_slice`. This scans invocation tokens (`func(...)`, `obj->func(...)`, `ns::func(...)`) using `tools::call_token_extractor`, filters language keywords, operators, and definition headers, and queries definitions. To guarantee zero false dependencies, the resolution strictly punts on any cross-file ambiguity (if a symbol has definitions across multiple distinct files, it is dropped rather than guessing the wrong implementation) and isolates test directories (non-test production files never depend on `tools/testing/`, `selftests/`, or `tests/`).
