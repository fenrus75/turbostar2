...
- **Hybrid GDB Coredump Backtrace Enrichment**: When generating `report.md`, `crashdump_manager` checks if a coredump file exists and GDB is available. If present, it invokes headless GDB (`--batch -ex "set width 0" -ex "set style enabled off" -ex "bt 35"`) to capture the full runtime call stack, parameter values (e.g. `foo (c=0x0)`), and accurate source lines. Frame numbers match interactive GDB (`agent_debug_coredump`) 1:1. Instrumentation frames up to `<signal handler called>` are labeled `crash handling` in a dedicated `Note` column to eliminate distraction without sacrificing diagnostic truth. Source paths are normalized relative to the project workspace to drive the compact Codemap summary. If no coredump exists or GDB fails, the report seamlessly falls back to the unwinder table.
