...
- **Crashdump Traceability:** Ensure crashes within the sandbox are captured via `systemd-crashdumpd` using predictable unit names, allowing easy retrieval via `crashdumpctl`.
...
| `project_hash` | `string` | `""` | A hash of the `project_dir`. Used to construct the `--unit=` argument for predictable crashdumps (e.g., `--unit=turbostar-project-<hash>`). This groups all jobs for a project under a predictable prefix. |
...
```bash
crashdumpctl --user info turbostar-project-<project_hash>-*
```
