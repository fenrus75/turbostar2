...
## 31-07-2026
- TUI Options Menu Restructuring into 3 Tabbed Dialogs (`src/ui/menu_bar.cpp`, `src/event_queue.h`, `src/editor_events.cpp`, `src/editor_events_ui.cpp`, `src/ui/dialog_factories.h/cpp`, `tests/unit/test_options_dialogs.cpp`): Consolidated the Options menu from 7 scattered items down to 3 unified tabbed dialog entries (`Editor Settings...`, `AI & Agent Settings...`, `A2A & Remote Settings...`). Created factory functions `create_editor_settings_dialog()`, `create_ai_settings_dialog()`, and `create_a2a_settings_dialog()` utilizing `ui_tabbed_container` with organized sub-pages (General & Code, Build & Run, LSP & Features, Syntax Colors; General AI, Model Providers, Task Models, MCP & Plugins; Server Security, Server Runtime, Remote Servers). Added unit test suite in `tests/unit/test_options_dialogs.cpp`. Verified with 100% test pass.
