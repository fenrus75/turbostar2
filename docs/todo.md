...
## 13-09-2026
- Website MCP Server subpage & navigation sync: created `docs/mcp.html` showcasing Turbostar's high-performance Model Context Protocol (MCP) server (`turbomcp` / `turbostar --mcp`). Featured value proposition for external AI agent CLIs (Gemini CLI, Claude Code, Antigravity, Cursor) with LSP X-ray codemaps, Semcode kernel indexing, compiler diagnostic minimization, and GDB crashdump analysis; added step-by-step installation with Meson (`meson install -C build` auto-creating the `turbomcp` symlink); provided practical configuration examples for Gemini CLI (`~/.gemini/config/mcp_config.json`), Claude Desktop/Code, and generic stdio MCP clients; documented command-line flags (`--project-dir`, `--log`, `--mcp`, `--yolo`, `--all-tool-families`); and added a 60+ tools catalog. Synchronized navigation headers and footers across `docs/index.html`, `docs/editor.html`, `docs/ai.html`, `docs/details.html`, and `docs/mcp.html`, added an MCP feature card to the homepage, and updated `docs/design-website.md`. (Completed)
- Fix type definition range collapse & Kind column formatting regressions: extracted `type_definition_cache::find_closing_brace_line` helper to scan forward from a start line tracking brace depth to find the matching closing `}` (with semicolon early-exit for forward declarations/typedefs); added `tools::expand_range_to_symbol_bounds` in `src/codemap_utils.{h,cpp}` to expand 1-line or single-point LSP definition ranges to factual multi-line symbol bounds via codemap symbols and closing brace scanning; used it in `semcode_backend::query_definition`, `semcode_backend::query_type_definition`, and `type_definition_cache::request_async`; corrected `semcode_backend::query_document_symbols` to report exact symbol kinds (Class, Struct, Enum, Interface) rather than hardcoding Function; restricted `underlying_type` in `semcode_indexer::ingest_types_stream` strictly to `typedef` and `alias` to stop stuffing member types into struct kinds and aborting brace scanning; and enabled fallback extraction of underlying aliases from source files in `type_definition_cache` when the indexer reports a typedef without pre-populated alias. Updated unit tests in `test_semcode_indexer`, `test_type_definition_cache`, and `test_semcode_backend`. (Completed)
- Semcode git checkout fallback & automatic background reindexing: enhanced `semcode_backend::init_indexer` to immediately open the newest available SQLite database as an active fallback when switching git commits so symbol navigation never stalls. If the current git commit HEAD is not yet indexed (`semcode_<git_head>.db` missing), `semcode_backend` automatically spawns a decoupled background worker thread (`semcode_bld`) to build and index the new HEAD asynchronously with `--reindex-if-stale`, atomic swap to the new index upon completion, and safe lifetime decoupling via `build_lifetime_state`. Added unit test in `tests/unit/test_semcode_backend.cpp` and updated `docs/design.md`. (Completed)
- Migrate `semcode_backend` to SQLite `semcode_indexer` and remove CLI scraping: migrated `semcode_backend` to directly query persistent SQLite B-tree databases produced by `semcode_indexer` instead of launching `semcode` CLI child processes and parsing command output text line-by-line. Removed the 20-thread background pre-warming loop, regex text scrapers, in-flight query deduplication condition variables, and CLI cache. Replaced them with instant in-process SQLite queries for outgoing call hierarchies, definitions, type inspections, and caller references (speeding up outgoing call lookups from 2,500ms to <1ms). Updated `semcode_indexer` with mutex thread-safety, caller queries, prefix searches, and automatic LRU cache pruning. Updated `src/meson.build`, `tests/unit/test_semcode_backend.cpp`, `tests/unit/test_call_resolver.cpp`, and `docs/design.md`. (Completed)

## 12-09-2026
- Strict ambiguity punting & test-path isolation for called dependencies: enforced zero false dependencies across `extract_outgoing_calls_from_slice` and `resolve_outgoing_call_target` in `src/codemap_utils.cpp`. When candidate definitions span multiple distinct files, the engine strictly punts on the symbol rather than guessing or tie-breaking between conflicting implementations (e.g. multi-arch variants or test mocks). Non-test caller files strictly reject dependencies pointing into test directories (`tools/testing/`, `selftests/`, `tests/`) and prevent cross-source `.c` file adoptions. In `get_outgoing_calls_in_range`, in-slice extraction seamlessly runs and merges missing unambiguous calls into the range result. Updated `tests/unit/test_semcode_backend.cpp` and `docs/design.md`. (Completed)
- Direct include tie-breaking & cross-file ambiguity punting in outgoing call extraction: implemented a two-tier disambiguation strategy in `extract_outgoing_calls_from_slice` in `src/codemap_utils.cpp`. When `semcode` or LSP returns multiple candidate definitions across different files, the engine parses `#include` directives in the calling file (`extract_included_header_suffixes`) and uses direct included headers as a clean, language-idiomatic tie-breaker. If multiple cross-file definitions remain unresolved, the call is cleanly punted rather than returning an incorrect architecture or helper definition. Expanded unit test coverage in `tests/unit/test_semcode_backend.cpp`. (Completed)
- Hardened in-slice outgoing call dependency extraction under `semcode_backend`: resolved false dependency reporting on function definition headers by passing `doc_symbols` to `extract_outgoing_calls_from_slice` in `src/codemap_utils.cpp` to filter candidate definitions on matching lines and pruning self-definitions targeting the same file. Fixed `is_project_file` to stop treating project-internal `include/` directories (e.g. Linux kernel `include/linux/*`) as external headers. Added `--git-repo` argument in `semcode_backend::run_semcode_query` to ensure git-aware semcode CLI queries target the intended repository regardless of working directory. Added regression tests in `tests/unit/test_semcode_backend.cpp` and `tests/unit/test_fs_file_codemap.cpp`. (Completed)
- In-slice call candidate extraction for outgoing dependencies (Approach A): enabled `fs_read_lines` to generate the `### Called Dependencies:` table under `semcode_backend` and language servers lacking LSP `callHierarchy/outgoingCalls` support. Added `src/call_token_extractor.h` / `src/call_token_extractor.cpp` to parse candidate call tokens from displayed line slices with comprehensive keyword and operator filtering, integrated definition resolution in `extract_outgoing_calls_from_slice` in `src/codemap_utils.cpp`, hardened `is_project_file` to recognize project-relative and test-isolated project roots, updated `meson.build`, added dedicated unit tests in `tests/unit/test_call_token_extractor.cpp`, and expanded `tests/unit/test_semcode_backend.cpp` and `tests/unit/test_fs_file_codemap.cpp`. (Completed)
- Website Semcode & Linux kernel AI documentation: featured Turbostar's hybrid Semcode indexing engine on `docs/index.html` (LSP feature card and quickstart prerequisites) and `docs/ai.html` (LSP X-ray vision details and Beyond the Baseline feature card), highlighting how agents and developers benefit from zero-lag semantic symbol navigation, struct layouts, and call hierarchies across massive codebases like the Linux kernel under strict Linux kernel AI rules. (Completed)
- `fs_read_lines` exact boundary preservation: fixed overshooting in `determine_adjusted_end_line` when reading lines ending exactly at the end of a function or block. Prevented symbol end expansion when `sym.end_line == requested_end` (which previously caused enclosing struct/class scopes to swallow subsequent functions), and prevented the fallback heuristic from scanning forward past `requested_end` if the line already terminates with a closing delimiter (`}`, `};`, `]`, `)`, `end`, `fi`, `done`). Added unit tests in `tests/unit/test_fs_file_codemap.cpp`. (Completed)
- LSP hover caching & non-blocking async fill-in: implemented asynchronous hover worker thread (`hover_worker_loop`) and request queue in `semcode_backend` to eliminate UI thread latency (reducing `request_hover` return time from >1000ms to <1ms on cache misses). Introduced `hover_cache_` in `standard_lsp_backend` to cache resolved hover payloads across both `standard_lsp_backend` and `semcode_backend` for instant (<1ms) cache hits on previously hovered tokens, invalidating entries on document updates. Added `cli_cache_` in `semcode_backend` to memoize CLI queries (`type`, `func`, etc.) and prevent redundant child process executions; updated unit tests in `tests/unit/test_semcode_backend.cpp` to verify non-blocking return, async event arrival, and instant cache hit retrieval. (Completed)
- Semcode CLI ANSI sanitization: replaced ad-hoc `strip_ansi_codes` helper in `src/semcode_backend.cpp` with `utf8::sanitize_terminal_output` to robustly strip terminal escape sequences across CLI output queries and hover event payloads; added unit test assertions in `tests/unit/test_semcode_backend.cpp`. (Completed)
- Semcode hybrid backend implementation (Step 2): implemented `semcode_backend` (`src/semcode_backend.h`, `src/semcode_backend.cpp`) inheriting from `standard_lsp_backend` with automatic on-demand activation when `.semcode.db` is present in the project root and `semcode-lsp` is available. Built hybrid architecture combining fast stdio JSON-RPC queries (`textDocument/definition` and `textDocument/references`) via `semcode-lsp` with semantic CLI operations (`semcode -q "type ..."` / `"func ..."` / `"callers -v ..."` / `"calls -v ..."`) for type inspection, function signatures, hover popups, outgoing call hierarchies, and workspace symbol discovery. Added executable path resolution (`fs_utils::find_executable`), working directory overlay optimization (skipping redundant full-file broadcasts for semcode-indexed languages), updated subclass documentation tables, documented hybrid architecture in `docs/design.md`, and added comprehensive unit tests in `tests/unit/test_semcode_backend.cpp`. (Completed)
...
- Default `CMAKE_EXPORT_COMPILE_COMMANDS` in CMake templates: added `set(CMAKE_EXPORT_COMPILE_COMMANDS ON)` to `templates/cmake_c/CMakeLists.txt` and `templates/cmake_cpp/CMakeLists.txt` so new CMake projects automatically output `compile_commands.json` for LSP / clangd indexing without requiring manual CLI flags; updated `src/meson.build` to declare `depend_files` for template files to ensure Ninja regenerates `project_templates_embedded.h` on template changes; and added unit test assertions in `tests/unit/test_project_template_manager.cpp`. (Completed)
- CMake project test discovery & CTest test listing support: extended `project_manager` to detect and list tests for CMake projects via `ctest --test-dir <build_dir> -N`, parsing test numbers and identifiers; updated build directory resolution (`resolve_build_dir`) to recognize CMake build artifacts (`CMakeCache.txt`, `CTestTestfile.cmake`, `Makefile`, `build.ninja`) in addition to Meson; tracked `CMakeLists.txt` mtime for automatic test cache invalidation on edits; updated `fs_run_tests` to use exact-match regex boundaries (`-R "^(...)$"`) with CTest; added shell resteering for `ctest -N` and `ctest --show-only` to point to `system://project/testlist.md`; and verified live test discovery against CMake project `fmt`. (Completed)
- `agent_get_run_terminaldump` ASCII Markdown screen dump & whitespace trimming: renamed and aliased `agent_get_run_screenshot` to `agent_get_run_terminaldump` (with aliases `agent_get_run_screenshot`, `agent_run_get_terminaldump`, and `agent_run_get_screenshot`) to clarify that output is ASCII terminal data rather than an image; redesigned output format from a 2D JSON array to clean Markdown with a fenced code block (` ``` `); trimmed trailing whitespace from each line and stripped trailing empty lines from the terminal screen to eliminate token bloat; and included cursor position and process status header (`process running` / `process ended`). Updated `docs/tools.md` and unit tests in `tests/unit/test_agent_get_run_screenshot.cpp`. (Completed)
- `run_executable` output capture support: added `output` boolean parameter (default: false) to `run_executable` allowing agents to capture and receive process execution output directly in the tool result JSON without resorting to terminal screen scraping. When `output` is true, recording is enabled on the run, execution waits up to `wait_for_time` (defaulting to 5s if omitted or 0), and sanitized output is returned under the `"output"` key. Documented in `docs/tools.md` and added unit test coverage in `tests/unit/test_run_executable.cpp`. (Completed)
