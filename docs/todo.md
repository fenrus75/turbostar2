...
## 01-08-2026
- `system://project/` VFS Directory & Removal of `fs_list_tests` (`src/vfs/system_vfs_provider.cpp`, `src/git_manager.h/cpp`, `src/tools/meson.build`, `meson.build`, `src/ui/agent_window.cpp`, `tests/unit/test_system_vfs.cpp`, `docs/tools.md`):
  1. Created `system://project/` virtual directory containing:
     - `system://project/diagnostics.md` (replaces `fs_compile_summary`).
     - `system://project/info.md` (overview of workspace root, upstream git remote URL, current branch, build system type, and `GEMINI.md`/`AGENTS.md` presence).
     - `system://project/testlist.md` (replaces `fs_list_tests`).
  2. Completely removed legacy tool `fs_list_tests` (`fs_list_tests.h`, `fs_list_tests_entry.cpp`, `fs_list_tests_security.cpp`) from `src/tools/` and `src/tools/meson.build`.
  3. Updated `meson.build` marking `unit_test_run_python` with `is_parallel: false`. Migrated test assertions to `test_system_vfs.cpp`. Updated `docs/tools.md` and `agent_window.cpp` prompt. Verified 100% test suite pass rate (253 OK, 1 Expected Fail, 2 Skipped).
