...
- Image Basic Operations Plugin: Implemented the `image-basic` shared module plugin, dynamically registering the `image` tool family and the `image_resize` and `image_crop` tools. Uses GraphicsMagick++ to scale images (by ratio or absolute dimensions) and crop selection rectangles (with X/Y offsets and canvas resetting). Modified images are resolved and re-ingested back into the `images://` VFS, supporting "edit in place" alias updates or writing to a new VFS output destination. Declared GraphicsMagick++ as an optional dependency so the plugin is dynamically built only when the library is detected on the system.
