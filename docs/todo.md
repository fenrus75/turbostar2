...
## 04-08-2026
- `fs_grep_files` Performance, Filtering, and Prioritization (`src/tools/fs_grep_files/`, `src/fs_security_manager.h/cpp`, `tests/unit/test_fs_grep_files.cpp`): Upgraded `fs_grep_files` to perform lightning-fast, high-performance regex searching across disk and open editor buffers. Key improvements include: (1) **Editor Buffer Priority**: Checks open document snapshots first before reading disk files, (2) **Tiered Priority Sorting**: Ranks results into 4 priority tiers (Tier 1 core code/headers, Tier 2 documentation, Tier 3 config/build files, Tier 4 backup/temp `~` files), ensuring core source files get detailed match listings before overflow files, (3) **Multi-Directory Traversal**: Expands searching across all VFS roots and workspace directories registered in `fs_security_manager`, (4) **Exclusion Filtering**: Added `exclude_path`, `exclude_ext`, and `exclude_pattern` (regex) parameters alongside automatic directory pruning (`.git`, `build`, `node_modules`), (5) **Overlapping Context Merging**: Merges adjacent and overlapping match context blocks into single contiguous listings. Added comprehensive unit tests covering all features in `tests/unit/test_fs_grep_files.cpp`.
