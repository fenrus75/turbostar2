...
## 01-08-2026
- Removal of TODO LLM Tools & Test Suites - Step 1 of 2 (`src/tools/`, `tests/unit/`, `tests/e2e/`, `src/tools/meson.build`, `meson.build`, `docs/tools.md`, `src/agentlib/llm_types.h`, `src/tools/enter_plan_mode/enter_plan_mode_entry.cpp`, `docs/todo.md`):
  1. Deleted 6 custom todo tool directories: `agent_add_todo`, `agent_list_todos`, `agent_complete_todo`, `agent_pop_todo`, `agent_delete_todo`, `get_subagent_todo_status`.
  2. Deleted corresponding unit and E2E test files (`test_agent_add_todo.cpp`, `test_agent_list_todos.cpp`, `test_agent_complete_todo.cpp`, `test_agent_pop_todo.cpp`, `test_agent_delete_todo.cpp`, `test_get_subagent_todo_status.cpp`, `test_pop_todo.cpp`, `test_agent_todos.py`, and references in `test_tools.cpp`).
  3. Removed build targets from `src/tools/meson.build` and `meson.build` (`agentcli_sources`). Updated `llm_types.h` tool alias map, `enter_plan_mode_entry.cpp` prompt text, and `docs/tools.md`.
