...
## 03-08-2026
- `command_runner` Writable `uv` Cache Exception & Strict Agent Profile Sync (`src/command_runner.h/cpp`, `docs/todo.md`):
  1. Extracted `add_cache_rw_exceptions()` helper in `command_runner.cpp` to create and add `~/.cache/uv`, `~/.uv`, `$UV_CACHE_DIR`, and `$XDG_CACHE_HOME/uv` (plus `ccache`) to `extra_rw_paths_`.
  2. Invoked `add_cache_rw_exceptions()` in both `apply_build_profile()` AND `apply_strict_agent_profile()` so that agent tool invocations (`run_python`, `run_shell_command`, MCP) receive writable `uv` cache exceptions (`BindPaths=` in sandboxed `tmpfs`).
  3. Added check preventing addition of `/tmp` or `/var/tmp` paths to `extra_rw_paths_` when running inside `test_watchdog` isolated `HOME` (where `PrivateTmp=true` provides a private writeable `tmpfs`).
  4. Added fallback to `std::filesystem::current_path()` when `project_dir_` is empty in `apply_build_profile()` and `apply_strict_agent_profile()`, ensuring working directories are properly mounted read-write in sandboxed `systemd-run` units.
