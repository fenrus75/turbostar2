...
- updated `delete_word_forward` (Ctrl-W) in the editor to stop at double quotes (`"`), semicolons (`;`), and commas (`,`) as word terminators in addition to spaces, and added comprehensive unit test coverage.
...
## 06-06-2026
- implemented a fast agent exit behavior on editor shutdown. The background summary worker thread's HTTP client transport is registered in `ai_agent::background_transport_` and cancelled via `cancel_current_task()` when the agent closes.
...
- added native stripping of `<think>...</think>` tags from assistant message content in Level 1 (and higher) history compaction to support local reasoning models. Added a comprehensive test case in `test_agent_compress_history.cpp`.
- implemented a new native agent tool `fs_man` in Turbostar that recursively searches for system man page files in `/usr/share/man`, follows `.so` redirects natively (with loop protection), decompresses `.gz` files using `zlib`, and renders them as Markdown using the existing `troff2md` converter. Added comprehensive unit tests in `test_fs_man.cpp`.
