...
## 01-08-2026
- `system://subagents/` VFS Migration & `agent://` Scheme Unification (`src/agentlib/ai_agent.cpp`, `src/ui/agent_window.cpp`, `src/tools/fs_read_lines/fs_read_lines_entry.cpp`, `src/vfs/system_vfs_provider.cpp`, `tests/unit/test_system_vfs.cpp`, `meson.build`, `docs/todo.md`):
  1. Consolidated all subagent completion reports and interaction logs under `system://subagents/` VFS endpoints (`system://subagents/<id>/final_result.md` and `system://subagents/<id>/transcript.md`).
  2. Fully deprecated and removed legacy `agent://` scheme references across codebase. Updated `ai_agent` completion event JSON payload and parent notification message to point directly to `system://subagents/<id>/final_result.md` and `system://subagents/<id>/transcript.md`.
  3. Updated system prompt VFS guidance table in `agent_window.cpp` featuring `system://` as the unified scheme covering documentation, workflow standards, project overview (`system://project/`), and active subagents (`system://subagents/`).
  4. Verified 100% test suite pass rate (253 OK, 1 Expected Fail, 2 Skipped).
