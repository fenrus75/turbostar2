...
- Image Basic Operations Plugin: Implemented the `image-basic` shared module plugin, dynamically registering the `image` tool family and the `image_resize` tool. Uses GraphicsMagick++ to scale images by ratio or absolute dimensions (with aspect-ratio preservation), dynamically resolving URIs and re-ingesting modified images back into the `images://` VFS with "edit in place" alias shifting. Declared GraphicsMagick++ as an optional dependency so the plugin is dynamically built only when the library is detected on the system.
