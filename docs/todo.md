...
## 27-07-2026
- Subclass Header Documentation Guideline (`templates/meson_cpp/AGENTS.md`): Added subclass comment table documentation rules to standard project template guidelines so all new C++ projects automatically mandate keeping subclass mapping tables updated in base class headers.
- AI Agent & LLM Client `std::span` & `std::string_view` Refactoring (`src/agentlib/ai_agent.h/cpp`, `src/agentlib/llm_client.h/cpp`, `src/filter_registry.h/cpp`): Refactored `ai_agent::set_conversation` (`std::span<const message>`), `set_conversation_unlocked` (`std::span<const message>`), `page_out_context` (`std::string_view`), `page_out_prior_context` (`std::string_view`), `snapshot_episode` (`std::string_view`), `llm_client::send_chat` (`std::span<const message>`, `std::string_view`), `send_chat_stream` (`std::span<const message>`, `std::string_view`), `filter_registry::register_filter` (`std::string_view`), `unregister_filter` (`std::string_view`), `has_filter` (`std::string_view`), `apply_filter` (`std::string_view`), and `get_registered_filters` (`std::string_view`). Verified with 100% test suite pass (244/244 ok).
- Context DNN Milestone Classifier `std::span` & `std::string_view` Refactoring (`src/agentlib/context_dnn.h/cpp`): Refactored `predict_boundary` (`std::string_view`, `std::span<const float>`), `pool_text` (`std::span<const std::string>`), `tokenize` (`std::string_view`), `compute_crc32` (`std::string_view`), and `load_weights` (`std::string_view`). Included `<cstdint>`. Verified with `unit_context_dnn` unit test passing.
- Document & Codereview Manager `std::span` & `std::string_view` Refactoring (`src/document.h/cpp`, `src/document_selection.cpp`, `src/codereview_manager.h/cpp`): Refactored `document::set_lsp_highlights` (`std::span<const text_range>`), `set_lsp_diagnostics` (`std::span<const diagnostic_info>`), `insert_block` (`std::span<const line>`), `codereview_manager::verify_line_contents` (`std::string_view`, `std::span<const std::string>`), and `file_matches` (`std::string_view`). Verified with `unit_document` and `unit_codereview_manager` passing.
- Address Lookup & Crashdump Manager `std::span` & `std::string_view` Refactoring (`src/address_lookup.h/cpp`, `src/crashdump_manager.h/cpp`): Refactored `address_lookup::resolve_addresses` (`std::span<const uintptr_t>`), `run_command` (`std::span<const std::string>`), `parse_maps` (`std::string_view`), `resolve_address` (`std::string_view`), `check_binary_exists` (`std::string_view`), `crashdump_manager::format_crash_notification` (`std::span<const crashdump_info>`), `refresh` (`std::string_view`), and `get_crashdumps_for_cookie` (`std::string_view`). Verified with `unit_address_lookup` and `unit_test_crashdump_window` tests passing.
- Hex & Binary Highlighters `std::span<const uint8_t>` & `std::string_view` Refactoring (`src/hex/`): Refactored `hex_highlighter` base class interface and subclasses (`elf`, `jpeg`, `png`, `pdf`, `tar`, `zip`) and `hex_highlighter_registry` (`can_handle`, `parse`, `get_info`, `detect_highlighter`, `get_offset_by_name`) to accept `std::span<const uint8_t>` and `std::string_view`. Enables zero-copy binary buffer inspection and sub-range analysis. Unit test `unit_hex_highlighter` passed cleanly.
- `std::span<const T>` Guideline Rule Addition (`GEMINI.md`, `templates/meson_cpp/AGENTS.md`): Updated project coding guidelines to mandate preferring `std::span<const T>` for read-only contiguous sequence parameters over `const std::vector<T> &`, alongside `std::string_view` for read-only string parameters, eliminating temporary container allocations and enabling zero-copy sub-range slicing.
- MIME Type Detector `std::string_view` & Zero-Allocation Extension Optimization (`src/mime.h/cpp`): Refactored parameter signatures (`detect_file_type`, `from_extension`, `detect_file_description`) to take `std::string_view`. Optimized `from_extension()` to perform case-insensitive extension matching directly against `std::string_view` target extensions, eliminating heap string allocations on extension lookups. Verified with 100% test suite pass (244/244 tests ok).
- File System Utilities `std::string_view` & `noexcept` Modernization (`src/fs_utils.h/cpp`): Refactored parameter signatures across `fs_utils` (`set_override_project_dir`, `make_relative_to_project`, `is_binary_file`, `get_file_type`, `count_lines_in_file`, `get_compile_command_for_file`, `execute_command_sync`, `get_project_history_dir`, `is_valid_db_name`, `is_shell_safe`, `escape_shell_arg`, `unescape_string`, `is_safe_for_ui`, `shorten_filename`, `format_binary_output`) to accept `std::string_view`. Updated `escape_arg` template to accept `std::string_view` directly without allocating temporaries. Annotated non-throwing security and path predicates (`is_valid_db_name`, `is_shell_safe`, `is_safe_for_ui`) with `noexcept`. Verified with 100% test suite pass (244/244 tests ok).
