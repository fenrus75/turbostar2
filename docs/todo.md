...
## 01-08-2026
- `system://skills.md` & `system://diagnostics.md` VFS Endpoints (`src/vfs/system_vfs_provider.cpp`, `tests/unit/test_system_vfs.cpp`, `docs/tools.md`): Implemented dynamic read-only VFS endpoints replacing `list_skills` and `fs_compile_summary`:
  1. **`system://skills.md`**: Dynamically generates Markdown table of available agent skills, URIs, and descriptions (supports `?search=<query>`).
  2. **`system://diagnostics.md`** (aliases `compile_summary.md`, `compile-summary.md`): Dynamically generates Markdown table of active compiler errors, compiler warnings, and LSP diagnostics across workspace files (supports `?search=<query>`).
  3. Added purpose descriptions and path resolution rules to `system_vfs_provider.cpp`. Added unit test assertions in `test_system_vfs.cpp`. Updated `docs/tools.md`. Verified 100% test suite pass rate (255 OK, 1 Expected Fail, 2 Skipped).
