...
## 22-07-2026
- Robust Multi-Candidate Source Path Resolution (`agent_get_profile_details_entry.cpp`): Implemented `resolve_file_path` in `agent_get_profile_details` to search across `working_directory`, `project_root`, and `project_dir`. Fixes issue where `code` was `null` during background agent runs when the process execution directory differed from the project root.
