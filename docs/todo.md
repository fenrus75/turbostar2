...
## 22-07-2026
- Robust Multi-Candidate Source Path Resolution (`agent_get_profile_details_entry.cpp`): Implemented `resolve_file_path` in `agent_get_profile_details` to search across `working_directory`, `project_root`, and `project_dir`, automatically stripping leading `../` / `./` build prefixes and normalizing with `lexically_normal()`. Fixes issue where `code` was `null` when `addr2line` emitted build-relative paths like `../src/main.cpp`.
