...
## 01-08-2026
- `system://skills.md` & `system://diagnostics.md` VFS Endpoints & Removal of `list_skills` / `fs_compile_summary` (`src/vfs/system_vfs_provider.cpp`, `src/tools/meson.build`, `meson.build`, `tests/unit/test_system_vfs.cpp`, `docs/tools.md`):
  1. Implemented dynamic read-only VFS endpoints `system://skills.md` (replaces `list_skills`) and `system://diagnostics.md` (aliases `compile_summary.md`, `compile-summary.md`, `build/summary.md`; replaces `fs_compile_summary`).
  2. Completely removed legacy tool classes (`list_skills.h`, `list_skills_entry.cpp`, `list_skills_security.cpp`, `fs_compile_summary.h`, `fs_compile_summary_entry.cpp`, `fs_compile_summary_security.cpp`) from `src/tools/` and `src/tools/meson.build`.
  3. Removed obsolete `test_list_skills` and `test_fs_compile_summary` targets from `meson.build` and migrated test coverage to `tests/unit/test_system_vfs.cpp`. Updated `docs/tools.md`. Verified 100% test suite pass rate (253 OK, 1 Expected Fail, 2 Skipped).
