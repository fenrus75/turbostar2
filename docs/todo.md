...
## 03-08-2026
- `command_runner` Execution Logging & Absolute `UV_CACHE_DIR` (`src/command_runner.cpp`, `meson.build`, `docs/todo.md`):
  1. Added explicit `event_logger` logging in `command_runner::execute()` and `build_command()` recording raw commands, exact sandboxed `systemd-run` command lines, duration, and exit codes.
  2. Created `~/.cache`, `~/.cache/uv`, and `~/.uv` on host in `add_cache_rw_exceptions()` to ensure `systemd-run` bind-mount sources exist.
  3. Fixed `UV_CACHE_DIR` in `meson.build` to use absolute path (`meson.project_source_root() / '.turbostar' / 'uv_cache'`) so `uv` resolves offline packages regardless of test runner working directory.
