...
- Robust Multi-Candidate Source Path Resolution (`agent_get_profile_details_entry.cpp`): Updated `resolve_file_path` in `agent_get_profile_details` to prioritize `config_manager::get_instance().get_build_directory()`, fallback to standard build directories (`build/`, `builddir/`, `out/`, `cmake-build-debug/`), resolve relative DWARF paths against the build tree, and perform recursive filename search fallback under `project_root` so source lines (`code`) are reliably populated.
