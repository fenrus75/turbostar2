...
## 4-09-2026
- Single-crash direct toolcall notification: when exactly 1 crash occurs during execution across `fs_run_tests`, `fs_compile_project`, `fs_compile_file`, and `run_python`, notify the agent with the direct call to retrieve crash details (`crashdump_get_info(crash_id="<id>")`) rather than telling it to call `crashdump_list` first. When multiple crashes occur, recommend `crashdump_list`. (Completed)
