...
## 16-07-2026
- Crash address logging in turbocatch: Implemented an async-signal-safe pointer-to-hex string formatter `safe_ptoa()` in `crash_catcher.c` and updated `write_info()` to capture the faulting memory address from `info->si_addr` when a signal is caught. The address is now written to `info.txt` under `CrashAddress: <address>` to make identifying NULL pointer dereferences much easier. Added test assertions to `test_assert_fail.cpp` verifying the new parameter.
