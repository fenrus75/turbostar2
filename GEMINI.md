...
- **CRITICAL**: Every unit test that tests settings, configuration, dialog state, or disk persistence MUST isolate the `HOME` environment variable to a temporary directory (e.g. `std::string temp_home = (std::filesystem::temp_directory_path() / "test_name_home").string(); std::filesystem::create_directories(temp_home + "/.cache/turbostar"); setenv("HOME", temp_home.c_str(), 1);`). Failing to isolate `HOME` in unit tests will overwrite or wipe out the user's actual live configuration files in `~/.cache/turbostar/`.
