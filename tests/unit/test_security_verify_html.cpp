...
extern "C" void register_security_verify_html(void);
extern "C" void unregister_security_verify_html(void);
...
int main()
{
	...
	// Ensure the security verify HTML tool is registered
	...
	std::cout << "Testing security_verify_html tool..." << std::endl;
	...
	sec_active = false;
	std::string valid_args = "{\"paths\": [\"docs/index.html\"]}";
	{
		auto prep = registry.prepare_tool("security_verify_html", valid_args, ctx);
	...
	// 2. Verify invalid parameters schema (empty paths list)
	{
		...
		auto prep = registry.prepare_tool("security_verify_html", args, ctx);
	...
	{
		std::string args = "{\"paths\": [\"nonexistent_html_file.html\"]}";
		auto prep = registry.prepare_tool("security_verify_html", args, ctx);
	...
	// 5. Test scanning a small valid dummy HTML file
	{
		...
		std::string res = registry.execute_tool("security_verify_html", args, ctx);

		std::cout << "HTML Tidy output for dummy file:\n" << res << std::endl;

		// Clean up the dummy file
		std::filesystem::remove(dummy_file_abs);

		// Output should indicate success or show tidy warnings/errors
		assert(res.find("passed successfully") != std::string::npos || res.find("Warning:") != std::string::npos || res.find("Error:") != std::string::npos);
	}

	// Clean up registration
	unregister_security_verify_html();

	std::cout << "security_verify_html tool verified successfully!" << std::endl;
	return 0;
}
