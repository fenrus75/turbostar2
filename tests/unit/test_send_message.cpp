...
int main()
{
	...
	std::string create_res = registry.execute_tool("invoke_subagent", "{\"name\": \"sub1\", \"task\": \"Test subagent\"}", ctx);
	...
	std::cout << "Testing send_message..." << std::endl;

	// 2. Success case: message the subagent
	{
		nlohmann::json args = {{"id", sub_id}, {"message", "hello subagent"}};
		std::string result = registry.execute_tool("send_message", args.dump(), ctx);
	...
	// 3. Validation failure: empty message
	{
		...
		auto prep = registry.prepare_tool("send_message", args.dump(), ctx);
	...
	// 5. Validation failure: null active agent context
	{
		...
		auto prep = registry.prepare_tool("send_message", args.dump(), null_ctx);
		assert(prep.tool == nullptr);
	...
	std::cout << "send_message tests passed successfully.\n";
	return 0;
}
