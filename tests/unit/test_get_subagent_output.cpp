...
#include "tools/get_subagent_output/get_subagent_output.h"
...
int main()
{
	...
	std::cout << "Testing get_subagent_output..." << std::endl;
	{
		// 1. Spawn subagent
		...
		std::string result = registry.execute_tool("get_subagent_output", std::format("{{\"id\": {}, \"keep\": true}}", sub_id), ctx);
		...
		result = registry.execute_tool("get_subagent_output", "{\"id\": 9999}", ctx);
		assert(result.find("Error") != std::string::npos);

		// 6. Reject missing ID
		{
			auto prep = registry.prepare_tool("get_subagent_output", "{}", ctx);
		...
		// 7. Reject if agent is read-only and keep is false (default is false)
		{
			...
			auto prep = registry.prepare_tool("get_subagent_output", std::format("{{\"id\": {}}}", sub_id), ctx);
			assert(prep.tool == nullptr);
			assert(prep.error_message.find("read-only") != std::string::npos);

			// Directly test validate_runtime on tool with read-only
			tools::get_subagent_output_tool direct_tool({sub_id, false});
		...
		std::cout << "get_subagent_output tool verified successfully!" << std::endl;
	}
