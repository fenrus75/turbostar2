...
void test_confirm_tool_execution()
{
	...
	// Verify parent agent received NO context injection (confirm is silent bookkeeping by design)
	auto parent_history = agent->get_conversation();
	for (const auto &msg : parent_history) {
		assert(msg.content.find("Subagent confirmed/verified code review item") == std::string::npos);
	}
