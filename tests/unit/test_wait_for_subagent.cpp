...
int main()
{
	...
	// Create subagent
	std::string create_res = registry.execute_tool("invoke_subagent", "{\"name\": \"sub_wait\", \"task\": \"Perform task\"}", ctx);
	...
	std::cout << "Testing wait_for_subagent..." << std::endl;

	// 1. Success case: wait for the subagent
	{
		nlohmann::json args = {{"id", sub_id}};
		std::string result = registry.execute_tool("wait_for_subagent", args.dump(), ctx);
	...
	// 3. Validation failure: missing active agent
	{
		...
		nlohmann::json args = {{"id", sub_id}};
		auto prep = registry.prepare_tool("wait_for_subagent", args.dump(), null_ctx);
	...
	// 4. Validation failure: unexpected arguments (should fail validation as per review recommendations)
	{
		...
		auto prep = registry.prepare_tool("wait_for_subagent", args.dump(), ctx);
		assert(prep.tool == nullptr);
	...
	std::cout << "wait_for_subagent tests passed successfully.\n";
	return 0;
}
