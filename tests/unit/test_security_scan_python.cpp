...
extern "C" void register_security_scan_python(void);
extern "C" void unregister_security_scan_python(void);
...
int main()
{
	...
	// Ensure the security scan Python tool is registered
	...
	std::cout << "Testing security_scan_python tool..." << std::endl;
	...
	sec_active = false;
	std::string valid_args = "{\"paths\": [\"tests/e2e/test_sqlite_tools.py\"]}";
	{
		auto prep = registry.prepare_tool("security_scan_python", valid_args, ctx);
	...
	// 2. Verify invalid parameters schema (empty paths list)
	{
		...
		auto prep = registry.prepare_tool("security_scan_python", args, ctx);
	...
	{
		std::string args = "{\"paths\": [\"nonexistent_python_script.py\"]}";
		auto prep = registry.prepare_tool("security_scan_python", args, ctx);
	...
	// 5. Test scanning a small valid dummy Python file
	{
		...
		std::string res = registry.execute_tool("security_scan_python", args, ctx);

		std::cout << "Bandit output for dummy file:\n" << res << std::endl;

		// Clean up the dummy file
		std::filesystem::remove(dummy_file_abs);

		// The output should be a valid JSON containing 'results' or 'metrics'
		assert(res.find("\"results\":") != std::string::npos);
		assert(res.find("\"metrics\":") != std::string::npos);
	}

	// Clean up registration
	unregister_security_scan_python();

	std::cout << "security_scan_python tool verified successfully!" << std::endl;
	return 0;
}
