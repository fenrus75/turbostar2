...
int main()
{
	...
	std::cout << "Testing get_subagent_todo_status..." << std::endl;
	{
		...
		{
			std::string res = registry.execute_tool("get_subagent_todo_status", "{\"id\": 101}", ctx);
		...
		{
			std::string res = registry.execute_tool("get_subagent_todo_status", "{\"id\": 999}", ctx);
		...
		{
			auto prep = registry.prepare_tool("get_subagent_todo_status", "{\"id\": 101, \"extra_arg\": true}", ctx);
		...
		// 6. Runtime validation case: reject if no active agent context
		{
			...
			auto prep = registry.prepare_tool("get_subagent_todo_status", "{\"id\": 101}", ctx);
			assert(prep.tool == nullptr);
			assert(!prep.error_message.empty());
		}

		std::cout << "get_subagent_todo_status tool verified successfully!" << std::endl;
	}
