...
extern "C" void register_security_scan_c(void);
extern "C" void unregister_security_scan_c(void);
...
int main()
{
	...
	// Ensure the security scan C tool is registered
	...
	std::cout << "Testing security_scan_c tool..." << std::endl;
	...
	{
		auto prep = registry.prepare_tool("security_scan_c", valid_args, ctx);
	...
	// 2. Verify invalid parameters schema (empty paths list)
	{
		...
		auto prep = registry.prepare_tool("security_scan_c", args, ctx);
	...
	{
		std::string args = "{\"paths\": [\"nonexistent_c_file.cpp\"]}";
		auto prep = registry.prepare_tool("security_scan_c", args, ctx);
	...
	// 5. Test scanning a C++ file
	{
		std::string dummy_file = "ts_sec_dummy.cpp";
		std::string dummy_file_abs = project_root + "/" + dummy_file;
		std::ofstream out(dummy_file_abs);
		out << "int main() { int x; return 0; }\n"; // uninitialized variable
		out.close();

		std::string args = "{\"paths\": [\"" + dummy_file + "\"]}";
		std::string res = registry.execute_tool("security_scan_c", args, ctx);

		std::cout << "cppcheck output for dummy file:\n" << res << std::endl;

		// Clean up the dummy file
		std::filesystem::remove(dummy_file_abs);

		// The output should be a valid XML containing 'results' or 'errors'
		assert(res.find("<?xml") != std::string::npos);
	...
	std::cout << "security_scan_c tool verified successfully!" << std::endl;
	return 0;
}
