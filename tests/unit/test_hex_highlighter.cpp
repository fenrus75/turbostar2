...
void test_elf_highlighter()
{
	...
	std::copy(strtab_name.begin(), strtab_name.end(), &data[strtab_start + 7]);

	// 4. Shdr [0] - Null Section (128..191) is all zeroes

	// 5. Shdr [1] - .text Section (192..255)
	size_t sh1_start = 128 + 64;
	write_u32_le(data, sh1_start, 1);	      // sh_name = 1 (".text")
	...
	write_u32_le(data, sh2_start, 7);	 // sh_name = 7 (".shstrtab")
	...
	write_u64_le(data, sh2_start + 32, 20);	 // sh_size = 20
	...
	inf = hl.get_info(data, 192); // sh_name field of SHT[1] (.text)
	assert(inf.type == hex_semantic_type::sect_header);
	assert(inf.description.find("sh_name") != std::string::npos);
	assert(inf.description.find(".text") != std::string::npos);

	inf = hl.get_info(data, 196); // sh_type field of SHT[1] (.text)
	assert(inf.type == hex_semantic_type::sect_header);
	...
	assert(inf.description.find("Sec \".text\"") != std::string::npos);
